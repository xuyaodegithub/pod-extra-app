...
import { timeFormatter, getCurrentLocalTime } from '@/app/lib/utils'
...
export default function TagCard() {
  ...
  const tabList = [
    {
      label: timeFormatter(Date.now()),
    },
    {
      label: 'AI-processed content',
    },
    {
      label: 'Economic Utility',
    },
    {
      label: 'Data Privacy',
    },
  ...
  const cardList = [
    {
      ...
      tags: ['inspirations', ' AI-processed content', 'inspirations', 'Summary'],
    },
    {
    ...
    <div className={``}>
      <div className={`flex flex-wrap mb-[10px]`}>
        {tabList.map((tab, index) => (
          <div
            className={`cursor-pointer text-sm px-[5px] mr-[10px] mb-[10px] py-[4px] rounded-5px ${isDark ? 'bg-bgDark text-white' : 'bg-bgGray text-fontGry-600'}`}
            key={index}
          >
            {tab.label}
          </div>
      ...
      <div className={``}>
        <div className={`max-h-[500px] overflow-auto relative`} ref={scrollRef}>
          {cardList?.map((card, index) => <TagCardItem card={card} key={index} />)}
        </div>
        <Link
          href={``}
