...
export function Shownotes({ data, goThisTime }: { data: any; goThisTime?: any }) {
  ...
  return (
    <div key="Shownotes">
      ...
      <div className={`text-md text-fontGry-600 dark:text-homehbg ShownotesBox mb-[40px]`} onClick={(e) => clickSpan(e)}></div>
      {!!tags?.length && (
      ...
      {!!relatedEpisodes?.length && (
        <div className={`pb-[30px]`}>
          ...
          <div className={`flex flex-wrap`}>{relatedEpisodes?.map((item: any, ind: number) => <TagCardItem key={ind} card={item} />)}</div>
        </div>
      )}
    </div>
  )
