...
export const metadata: Metadata = getMetaData({
  ...
  keywords: '',
  alternates: {
    canonical: `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/podcasts-categories`,
  },
})
export default async function Page() {
  ...
  return (
    <main className={`flex flex-col`}>
      <div className={`border border-gray-1000 rounded-10px p-[25px] flex-1 mt-[3px] dark:border-fontGry-600`}>
        {resultList.map(({ categoryName, children, categoryId, categoryUrl = '' }: any) => {
          return (
            <div className={`mb-[25px]`} key={categoryId}>
              ...
              <div className={`flex flex-wrap text-sm text-fontGry-600`}>
                {children?.map(({ categoryName: childName, categoryId: childId, categoryUrl = '' }: any): any => (
