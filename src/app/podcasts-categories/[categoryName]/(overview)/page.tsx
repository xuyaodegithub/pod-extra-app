...
import { getNoTagText, PUB_DATE, POPULARITY, getCurrentLocalTime } from '@/app/lib/utils'
        ...
        {resultList.map(({ coverUrl, categoryList, showId, itunesAuthor, showTitle, showDescription, gmtLastUpdate }: any) => {
          ...
          return (
            <Link href={`/podcast/${encodeURIComponent(title)}-podcast-${showId}`} key={showId} className={`w-[50%]`}>
              <div className={`flex mb-4px cursor-pointer overflow-hidden hover:bg-hbg transition-all p-[10px] rounded-10px`}>
